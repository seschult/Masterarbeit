...
\frame{
	\frametitle{\bfseries{Event yields after preselection}}
...
%	\begin{alertblock}
	...
	\vspace{0.2cm}
	$\Rightarrow$ \bfseries{Chose best  permutations for calculation} 
...
\frame{
	\frametitle{\bfseries{3D-template technique}}
		...
		\begin{block}{\bfseries{determination of $m_{top}, JSF$ and $bJSF$ }}
		\begin{itemize}
			\item absorb data/MC mean deviations into the $JEF$ and $bJSF$
			\vspace{0.2cm}
			\item $JES/bJES$ induced uncertainties, become an additional statical component
			\vspace {0.2cm}
			\item different masses in the  range 170?175.5 GeV + independent input  for  JSF/bJSF 0.96-1.04
	...
	\begin{block}{\bfseries{template construction}}
		\begin{itemize}
			\item  templates are derived for $m_{top}^{reco}, m_{top}^{reco}$ and $R_{bq}^{reco}$ 
			\vspace{0.2cm}
		
			\item  fit of signal and background with different functions
...
\end{textblock}
	...
	\begin{block}{\bfseries{fit-functions (signal)}}
		\begin{itemize}
...
\frame{
	\frametitle{\bfseries{signal templates $t\={t}$ only for 170 GeV $\&$ 171.5 GeV }}
...
\frame{
	\frametitle{\bfseries{Next steps }}
	
		\begin{textblock}{15.5}(0.5,2.75)
		\begin{itemize}
			\item single top contains additional information $\Rightarrow$ add to signal 
			\vspace {0.2cm}
			\item Several fits with variation of  $JFS$ and$ bJSF$
			\vspace{0.2cm}
			\item Repetition of the   parametrisation for the different Background  processes 
			\vspace{0.2 cm}
			\end{itemize}
				\\
		...
		\end{textblock}	
		\begin{textblock}{17.5}(1.5,6.75)
				\begin{alertblock}{
				$\Rightarrow$ \bfseries{Dependes of $m_{top}^{reco}, m_{top}^{reco}$ and $R_{bq}^{reco}$  on $mtop, JSF \/bJSF$ }}
			\end{alertblock}
			
		\end{textblock}
	
\begin{textblock}{14.5}(0.5,8.75)	
	\begin{itemize}
		\item Probability density functions for signal and background are used for the unbined Likelihood fit
	\end{itemize}
...
\frame{
	\frametitle{\bfseries{Summery}}
	\begin{textblock}{18}(5, 2.5)	
