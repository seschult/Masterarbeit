...
\begin{textblock}{15}(2, 9.5)	
		...
		\vspace{0.2cm}
			...
			\vspace{0.2cm}
			\item Significant contributions to radiative corrections 
			\vspace{0.2cm}
...
\frame{
	\frametitle{\bfseries{Previous results}}
		\begin{textblock}{6.5}(0.5, 4.7)	
			...
			\begin{alertblock}{l+jets /3D-template method:}
				\begin{itemize}
					\item 7 TeV: \href{https://twiki.cern.ch/twiki/bin/viewauth/AtlasProtected/TopMassNtuplesRun2}{\beamergotobutton{Top Mass Ntuple production}}	
					\vspace{0.2cm}
						$m_{top} = 172.33 \pm 1.27$ GeV
...
\frame{
	\frametitle{\bfseries{3D-template technique}}
		...
		\begin{itemize}
			...
			\vspace{0.2cm}
			$\Rightarrow JES/bJES$  uncertainties, become an additional statisticall component
			\vspace {0.2cm}
	...
	\begin{block}{\bfseries{Fit (signal)}}
		\begin{itemize}
			\item $m_{top}^{reco}$:   gauss+ landau + landau$_{mirrored}$
		\vspace{0.2cm}
...
\frame{
	...
	\begin{textblock}{15}(0.5, 2.0)	
		\begin{alertblock}{Current status}
			\begin{itemize}
				\item Event selection and reconstruction with 13 TeV samples \\
				\vspace{0.1cm}
				$\Rightarrow$ good data/MC agreement, except for b-tagging multiplicity, worse agreement for four jets, two b-tagged inclusive 
				\vspace{0.2cm}
				\item Template parametrisation for several $t\={t}$ signal samples \\
					\vspace{0.1cm}
		...
		\end{textblock}	
			...
			\begin{alertblock}{Next steps}
				\begin{itemize}
					...
					\item Perform the parametrization for all $JSF$ and $bJSF$\\
