...
TemplateHolder :: TemplateHolder (const char *File){
    ...
    f13 = new TF1("f3",gauss,56,110,3);
    f14 = new TF1("f4",gauss,56,110,3);
	fw = new TF1("fmw",fit_mw,56,110,6); 
...
void big_fw_fcn(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
double chisq[200];
double chisqG=0;
...
for (int k=0;k<6;k++) localpar[k]=par[4*k+0]+par[4*k+1]*(gParameters[j][0]-172.5)+par[4*k+2]*(gParameters[j][1]-1.0)+par[4*k+3]*(gParameters[j][2]-1.0);
...
void big_frbq_fcn(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
double chisq[200];
double chisqG=0;
...
for (int j=0;j< gALL.size();j++)
{
double rbqtemplate=gParameters[j][0];	
...
for (int k=0;k<9;k++) localpar[k]=par[4*k+0]+par[4*k+1]*(rbqtemplate-172.5)+par[4*k+2]*(gParameters[j][1]-1.0)+par[4*k+3]*(gParameters[j][2]-1.0);
...
int ftop_npar=36;
double chi2_top=0;
big_ftop_fcn(ftop_npar, 0, chi2_top, par,iflagq);
int fw_npar=24;
double chi2_w=0;
big_fw_fcn(fw_npar, 0, chi2_w, &par[36],iflagq);
int frbq_npar=36;
double chi2_rbq=0;
big_frbq_fcn(frbq_npar, 0, chi2_rbq, &par[36+24],iflagq);
...
void TemplateHolder :: top_fit(){
	...
	arglistr[0] = 1500000;
	arglistr[1] = 0.0001;
    TMinuit *aMinuit = new TMinuit(6*4+9*4+9*4); 
  
	aMinuit->SetFCN(big_all_fcn);
	int q=0;
  
  	aMinuit->mnparm(q, Form("a%i",q), 1.10618e+04, 1000.0, 1000,1000000,ierflg); q++;//zentralparameter
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;//tompmasse
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;//bJSf
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;//JSF
	aMinuit->mnparm(q, Form("a%i",q), 1.62284e+02, 10, 10,1000.0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 1.28988e+01, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 4.28790e+05, 0.1, 1000,10000000,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 1.87218e+02, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 1.48783e+01, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 4.46906e+05, 0.1, 1000,1000000.0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q),  1.36078e+02, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 1.02262e+01, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflg); q++;
    ...
    aMinuit->mnparm(q, Form("a%i",q),  2.48333e+04  , 10., 0.0,10000000,ierflgw); q++;//zentralparameter
	aMinuit->mnparm(q, Form("a%i",q), -5.36697e+02, 0.1, -100000,10000000,ierflgw); q++;//aMasse
	aMinuit->mnparm(q, Form("a%i",q),0, 0.1, -100000,10000000,ierflgw); q++;//bJSf
	aMinuit->mnparm(q, Form("a%i",q), 0, 0.1, -1000,10000000,ierflgw); q++;//JSF
	aMinuit->mnparm(q, Form("a%i",q), 8.10386e+01   , 10, 40,100,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q),   2.01724e-02 , 0.1, 0,0,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0, 0.1, 0,0,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q), 6.79767e+00 , 0.1, 1.0,15,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgw); q++;
	
	aMinuit->mnparm(q, Form("a%i",q), 1.77091e+04, 1.0, 100.0,10000000,ierflgw); q++;//zentralparameter
	aMinuit->mnparm(q, Form("a%i",q), -3.28105e+02 , 0.1, -1000,1000,ierflgw); q++;//aMasse
	aMinuit->mnparm(q, Form("a%i",q),0, 0.1, -100000,100000,ierflgw); q++;//bJSf
	aMinuit->mnparm(q, Form("a%i",q), 0, 0.1, -100000,100000,ierflgw); q++;//JSF
	aMinuit->mnparm(q, Form("a%i",q), 8.39734e+01, 5, 70,120,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q), -2.22161e-02, 0.1, 0,0,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0., 0.1, 0,0,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q), 1.70114e+01, 0.01, 10.0,50,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q), -4.48616e-02, 0.1, 0,0,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgw); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgw); q++;


  
  	aMinuit->mnparm(q, Form("a%i",q), 4.51153e+03, 1000.0, 1000,1000000,ierflgr); q++;//zentralparameter
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.01, 0,0,ierflgr); q++;//tompmasse
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.01, 0,0,ierflgr); q++;//bJSf
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.01, 0,0,ierflgr); q++;//JSF
	aMinuit->mnparm(q, Form("a%i",q), 1.92480e+00, 10, 0,1000.0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 9.04483e-01, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 6.16592e+03 , 0.1, 1000,10000000,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 1.17703e+00, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 5.52227e-01, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 3.10282e+04, 0.1, 1000,1000000.0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 7.36829e-01, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 2.44623e-01, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
	aMinuit->mnparm(q, Form("a%i",q), 0.0, 0.1, 0,0,ierflgr); q++;
  	...
  	aMinuit->mnexcm("MIGRAD", arglistr ,2,ierflgr);
	aMinuit->mnexcm("SIMPLEX", arglistr ,2,ierflgr);
	aMinuit->mnexcm("MIGRAD", arglistr ,2,ierflgr);
	aMinuit->mnexcm("SIMPLEX", arglistr ,2,ierflgr);
	aMinuit->mnexcm("MIGRAD", arglistr ,2,ierflgr);
	aMinuit->mnexcm("SIMPLEX", arglistr ,2,ierflgr);
	...
	aMinuit->mnexcm("HESSE", arglistr ,2,ierflgr);
...
for(size_t i=0; i<samples.size(); i++) { 
	...
	gALL[i]->mtop_plot(Form("./out_JSF_%2.2f_bJSF_%2.2f_mtop_%3.1f",gParameters[i][0],gParameters[i][1],gParameters[i][2]));
;
