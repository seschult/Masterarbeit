...
void TemplateHolder::save(std::string output){
	
	std:string out_file = output + "3DPara_output.root";
	
	TFile *fend = new TFile(out_file.c_str(),"RECREATE");
	
	
	TTree *t1 = new TTree("out","Data");
	t1->Branch("fmtop",&fmtop,"fmtop/D");
	t1->Branch("fJSF",&fJSF,"fJSF/D");
	t1->Branch("fbJSF",&fbJSF,"fbJSF/D");
	
	t1->Branch("fit_mtop_par",&par_mtop,"fit_mtop_par/D");
	t1->Branch("fit_mtop_err",&err_mtop,"fit_mtop_err/D");
	t1->Branch("fit_mw_par",&par_mw,"fit_mw_par/D");
	t1->Branch("fit_mw_err",&err_mw,"fit_mw_err/D");
	t1->Branch("fit_rbq_par",&par_mrbq,"fit_rbq_par/D");
	t1->Branch("fit_rbq_err",&err_mrbq,"fit_rbq_err/D");
